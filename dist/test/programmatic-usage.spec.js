...
import { addCommentThatCodeIsGenerated, schema2typebox, } from "../src/programmatic-usage.js";
import { buildOsIndependentPath, expectEqualIgnoreFormatting, } from "./util/index.js";
...
describe("programmatic usage - when running the programmatic usage", () => {
    ...
    `);
        await expectEqualIgnoreFormatting(await schema2typebox({ input: dummySchema }), expectedTypebox);
    });
   ...
   `;
    ...
    `);
            ...
            zip(inputPaths, [referencedPersonSchema, referencedStatusSchema]).map(([fileName, data]) => {
    ...
    `);
            ...
            zip(inputPaths, [referencedCatSchema, referencedDogSchema]).map(([fileName, data]) => {
                return fs.writeFileSync(fileName, JSON.stringify(data), undefined);
            });
            await expectEqualIgnoreFormatting(await schema2typebox({ input: JSON.stringify(dummySchema) }), expectedTypebox);
            // cleanup generated files
            const { code: returnCode } = shell.rm("-f", inputPaths);
            expect(returnCode).toBe(SHELLJS_RETURN_CODE_OK);
        });
